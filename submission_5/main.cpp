...
static void printStastics() {
    ...
    std::cout << "Comparisons: " << stats.comparisonCount << ", Swaps: " << stats.swaps << ", Time taken : " << stats.timeTaken << " seconds." << std::endl;
}
...
private:
    ...
    TBankAccount** array;
    int size;
    TBankAccount** sortedArr;
    bool isSorted;

    void swap(TBankAccount* first, TBankAccount* second) {
        TBankAccount* temp = first;
        first = second;
        second = temp;
        stats.swaps++;
    }

    int Partition(TBankAccount** arr, int low, int high, FCompareAccounts compareFunc) {
        TBankAccount* pivot = arr[high];
        int i = low - 1;
        for (int j = low; j < high; j++) {
            ...
            if (compareFunc(arr[j], pivot) <= 0) {
                i++;
                swap(arr[i], arr[j]);
                TBankAccount* temp = arr[i];
                arr[i] = arr[j];
                arr[j] = temp;
            }
        }
        swap(arr[i + 1], arr[high]);
        TBankAccount* temp = arr[i + 1];
        arr[i + 1] = arr[high];
        arr[high] = temp;
        return i + 1;
    }

    void QuickSortRecursive(TBankAccount** arr, int low, int high, FCompareAccounts compareFunc) {
        if (low < high) {
            int pi = Partition(arr, low, high, compareFunc);
            QuickSortRecursive(arr, low, pi - 1, compareFunc);
            QuickSortRecursive(arr, pi + 1, high, compareFunc);
        }
    }

    TLinkedListNode* MergeSortedLists()
};
